...
public:
    ...
    void printByLevel(Node<T> *cur); // prints out tree starting from node (level by level) *uses queue implementation*

    ~AVL(); // destructor

    // Needs to be done
    bool isValidBST(Node<T> *cur); 

    bool search(Node<T> *cur, T val); 

    int findDepth(Node<T> *cur, T val, bool found = false);                                 
    int findKeyHeight(Node<T> *cur, T val, bool found = false); 

    int numLeaves(Node<T> *cur); 
    int numNodes(Node<T> *cur);   

    Node<T> *findParent(Node<T> *cur, T val);          
    Node<T> *smallestNode(Node<T> *cur);           

    void printPreOrder(Node<T> *cur);  
    void printInOrder(Node<T> *cur);   
    void printPostOrder(Node<T> *cur);
};
...
template <typename T>
void AVL<T>::printByLevel(Node<T> *cur)
{
